                ...
                <div class="card-body py-4-5 px-4">
                    <div class="d-flex justify-content-end mb-4 gap-2">
                        @if (auth()->user()->isAdmin() && $bantuan->status === 'DIAJUKAN')
                            <x-modal.confirm route="{{ route('dashboard.bantuan.approve', $bantuan->uuid) }}" method="PATCH"
                                id="bantuan-disetujui" title="Konfirmasi" color="success">
                                <x-slot:btn>
                            ...
                            <x-modal.confirm route="{{ route('dashboard.bantuan.decline', $bantuan->uuid) }}" method="PATCH"
                                id="bantuan-ditolak" title="Konfirmasi" color="danger">
                                <x-slot:btn>
                        ...
                        @if (auth()->user()->isRelawan() && $bantuan->status === 'DISETUJUI')
                            <x-modal.confirm route="{{ route('dashboard.bantuan.received', $bantuan->uuid) }}"
                                method="PATCH" id="bantuan-diterima" title="Konfirmasi" enctype="multipart/form-data">
                                <x-slot:btn>
                                    <i class="bi bi-check-circle"></i>
                                    Diterima
                                </x-slot>
                                Tekan <b>KONFIRMASI</b> jika Bantuan telah diterima oleh penyandang atas nama
                                <b>{{ $bantuan->penyandang->nama }}</b>
                        ...
                        @if (auth()->user()->isRelawan() && ($bantuan->status === 'DIAJUKAN' || $bantuan->status === 'DITOLAK'))
                            <x-form.delete :id="$bantuan->uuid" :action="route('dashboard.bantuan.destroy', $bantuan->uuid)" :label="'Bantuan ' .
                                $bantuan->jenis .
                                ' yang diterima oleh ' .
                                $bantuan->penyandang->nama" text="Hapus" />
                        @endif
                    </div>
